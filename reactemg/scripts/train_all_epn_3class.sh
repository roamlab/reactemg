#!/bin/bash
# Train every model variant on EMG-EPN-612 with num_classes=3.
# 3-class config keeps {open, fist, relax} and drops {wavein, waveout, pinch}
# (the filtering happens automatically in get_csv_paths based on --num_classes).
#
...
#   source scripts/train_all_epn_3class.sh   # from reactemg/
#   bash   scripts/train_all_epn_3class.sh   # from anywhere
