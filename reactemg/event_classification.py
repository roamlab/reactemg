...
def main(
    ...
    if epn_eval == 1:
        epn_data_master_folder = "../EMG-EPN612_Dataset"
    ...
    for i, csv_path in enumerate(csv_paths_all):
        ...
        if epn_eval == 1:
            print(
                f"  EPN Class. Acc so far:    {partial_epn_class_acc:.4f} ± {partial_epn_class_std:.4f}"
            )
            print(
                f"  EPN Smooth Acc so far:    {partial_epn_smooth_class_acc:.4f} ± {partial_epn_smooth_std:.4f}"
            )
            print(
                f"  EPN Recog. Acc so far:    {partial_epn_recog_acc:.4f} ± {partial_epn_recog_std:.4f}"
            )
